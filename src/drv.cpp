  ...
  if (v_info.brake_stroke > v_config._BRAKE_PEDAL_OFFSET || v_info.control_mode != 1/*auto pilot mode*/) {
    ...
    */
    ...
    if (cmd_velocity > 10/***10**20160905***/) {
       target_accel_stroke = v_config._K_ACCEL_P_UNTIL20 * e + v_config._K_ACCEL_I_UNTIL20 * e_i +
       v_config._K_ACCEL_D_UNTIL20 * e_d + v_config._K_ACCEL_OFFSET;
    } else {
       target_accel_stroke = v_config._K_ACCEL_P_UNTIL10 * e + v_config._K_ACCEL_I_UNTIL10 * e_i +
       v_config._K_ACCEL_D_UNTIL10 * e_d + v_config._K_ACCEL_OFFSET;
    }
#else
...
void PedalControl(double current_velocity, double cmd_velocity)
{
   ...
   if ((fabs(cmd_velocity) + 1.5) >= current_velocity
       && (cmd_velocity != 0.0)) {
   ...
   } else if ((fabs(cmd_velocity) + 1.5) < current_velocity
